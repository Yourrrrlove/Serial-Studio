...
_HOTPATH_ASSERT_ALLOWED = (
    ...
    "core/Core/DataModel/DataBlock.h",
...
_BUS_ON_HOTPATH_RE = re.compile(
    r"Core/Bus/|MessageBus::instance|\b(?:publish|publishState|subscribe|latest)<"
)
...
def find_bus_on_hotpath_violations(
    ...
    """Flag bus traffic inside a hotpath TU (error): publish allocates, blocks stay pooled.

    A forward declaration and a `MessageBus&` handed in by the composition root are allowed
    (spec 0077 injection); an include of the bus headers or any publish/subscribe/latest call
    is not.
    """
    posix = path.as_posix()
...
_TRIAL_PARITY_ALLOWED = (
    "core/Ui/Console/WelcomeText.cpp",
...
def find_trial_parity_violations(
    ...
    posix = path.as_posix()
    if "app/src/Licensing/" in posix or "core/Core/Licensing/" in posix:
        return []
...
_API_GENERATED = {
    ...
    "core/Core/DataModel/Generated/DatasetSerialization.cpp": "never edit by hand",
    "core/Ui/ProjectEditor/Generated/DatasetForm.cpp": "never edit by hand",
...
_REGISTRY_KEY_ALLOWED = {
    ...
    # spec 0070).
    "core/Core/DataModel/Frame.h": "sibling-entity serializers",
    "core/Core/DataModel/FrameKeys.h": "Keys:: declarations",
    ...
    "core/Core/DataModel/Frame.cpp": "hand-written sub-entity readers declared as manifest hooks",
    ...
    "core/Core/DataModel/PropertyValidators.cpp": "the ProjectModel-free validators",
}
...
_CENSUS_LAYERS = ("Core", "Protocols", "Pipeline", "Devices", "Storage", "Api", "Ui")
_CLASS_DECL_RE = re.compile(
    r"^\s*(?:class|struct)\s+(?:Q_\w+\s+)?([A-Za-z_]\w*)\b(?!\s*;)", re.M
)


def _census_layer_of(rel: str) -> str:
    """The library a repo-relative source belongs to: a core/ layer, App or Tests."""
    parts = rel.split("/")
...
def _class_layer_index(repo_root: Path) -> dict[str, list[tuple[str, str]]]:
    """Maps every class or struct declared in a core/ or app/src header to (layer, path)."""
    index: dict[str, list[tuple[str, str]]] = {}
    for tree in (repo_root / "core", repo_root / "app" / "src"):
        for path in sorted(tree.rglob("*.h")):
            if "ThirdParty" in path.parts:
                continue
            rel = path.resolve().relative_to(repo_root).as_posix()
            layer = _census_layer_of(rel)
            try:
                text = path.read_text(encoding="utf-8", errors="replace")
            except OSError:
                continue
            for name in set(_CLASS_DECL_RE.findall(text)):
                index.setdefault(name, []).append((layer, rel))
    return index


def _resolve_reach_layer(qualified: str, caller_layer: str, index: dict) -> str:
    """The library that declares the class `X::instance()` reaches; Qt accessors are "Qt", an
    unknown class "Unknown". A name declared in several libraries is disambiguated by the
    namespace segments (a directory named like the namespace), then by the caller's own layer.
    """
    segments = qualified.split("::")
    simple = segments[-1]
    if re.match(r"^Q[A-Z]", simple):
        return "Qt"
    candidates = index.get(simple)
    if not candidates:
        return "Unknown"
    layers = {layer for layer, _ in candidates}
    if len(layers) == 1:
        return next(iter(layers))
    for layer, rel in candidates:
        if any(f"/{ns}/" in rel for ns in segments[:-1]):
            return layer
    if caller_layer in layers:
        return caller_layer
    return "Ambiguous"
...
def _collect_singleton_census(repo_root: Path) -> dict:
    ...
    cross_library: list[dict] = []
    index = _class_layer_index(repo_root)
    total = 0
    ...
    for path in sorted(iter_source_files(trees)):
        ...
        caller = _census_layer_of(rel)
        if caller not in _CENSUS_LAYERS:
            continue
        for line, name in stats.get("reaches", []):
            callee = _resolve_reach_layer(name, caller, index)
            if callee in ("Qt", caller):
                continue
            edge = f"{caller}->{callee}"
            per_edge[edge] = per_edge.get(edge, 0) + 1
            cross_library.append(
                {"path": rel, "line": line, "class": name, "layer": callee}
            )
    ...
    return {
        ...
        "files": len(per_file),
        "buckets": buckets,
        "cross_library_total": len(cross_library),
...
def _print_singleton_census(census: dict) -> None:
    ...
    print(
        f"\ncross-library reaches inside core/: {census.get('cross_library_total', 0)}"
    )
    for edge, count in census.get("per_edge", {}).items():
...
def _run_singleton_census(repo_root: Path, check: bool, accept: bool) -> int:
    ...
    if census.get("cross_library_total", 0):
        print(
            f"singleton census: {census['cross_library_total']} cross-library reach(es) inside "
            "core/ (spec 0077 AC3 requires zero outside the composition root):",
            file=sys.stderr,
        )
        for site in census["cross_library"][:60]:
            print(
                f"  {site['path']}:{site['line']}: {site['class']}::instance() -> {site['layer']}",
                file=sys.stderr,
            )
        print(
            "\nRead the module through the root-bound set that owns it (Core::services(), "
            "DataModel::pipelineModules(), API::handlerContext()) or take it by constructor.",
            file=sys.stderr,
        )
        return 1
...
_BUS_CENSUS_BASELINE = Path(__file__).with_name("bus-census.json")
_BUS_TOPIC_RE = re.compile(
    r"^struct\s+([A-Za-z_]\w*)\s+final\s*\{(.*?)\n\};", re.M | re.S
)
_BUS_TOKEN_RE = re.compile(
    r"\b(publish|publishState|subscribe|latest)<\s*(?:Core::Bus::)?([A-Za-z_]\w*)\s*>"
)
_BUS_OWN_FILES = (
    "Core/Bus/Messages.h",
    "Core/Bus/MessageBus.h",
    "Core/Bus/MessageBus.cpp",
)


def _bus_topics(messages: str) -> tuple[list[str], set[str]]:
    """Returns the topic names declared in Messages.h and the value types nested inside them."""
    structs = _BUS_TOPIC_RE.findall(messages)
    names = [name for name, _ in structs]
    value_types: set[str] = set()
    for _, body in structs:
        for name in names:
            if re.search(r"\b" + re.escape(name) + r"\b", body):
                value_types.add(name)
    return [n for n in names if n not in value_types], value_types


def _collect_bus_census(repo_root: Path) -> dict:
    """Counts every publish/subscribe/latest token per topic over core/ and app/src, and lists
    the structural defects: a topic nobody publishes or nobody reads, a token naming no topic,
    and any bus token inside a hotpath translation unit."""
    messages = (repo_root / "core" / "Core" / "Bus" / "Messages.h").read_text(
        encoding="utf-8", errors="replace"
    )
    topics, value_types = _bus_topics(messages)
    counts = {name: {"publishers": 0, "subscribers": 0} for name in topics}
    errors: list[str] = []
    for path in sorted(
        iter_source_files([repo_root / "core", repo_root / "app" / "src"])
    ):
        if path.suffix not in (".cpp", ".h") or "ThirdParty" in path.parts:
            continue
        rel = path.resolve().relative_to(repo_root).as_posix()
        if any(rel.endswith(own) for own in _BUS_OWN_FILES):
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        hotpath = any(rel.endswith(allowed) for allowed in _HOTPATH_ASSERT_ALLOWED)
        for match in _BUS_TOKEN_RE.finditer(text):
            verb, name = match.group(1), match.group(2)
            number = text.count("\n", 0, match.start()) + 1
            if hotpath:
                errors.append(
                    f"{rel}:{number}: bus token {verb}<{name}> in a hotpath TU"
                )
            if name not in counts:
                errors.append(
                    f"{rel}:{number}: {verb}<{name}> names no topic in Messages.h"
                )
                continue
            key = "publishers" if verb.startswith("publish") else "subscribers"
            counts[name][key] += 1
    for name in topics:
        if counts[name]["publishers"] == 0:
            errors.append(f"core/Core/Bus/Messages.h: topic {name} has no publisher")
        if counts[name]["subscribers"] == 0:
            errors.append(f"core/Core/Bus/Messages.h: topic {name} has no subscriber")
    return {
        "spec": "0077-independent-modules",
        "regenerate": "python scripts/code-verify.py --bus-census --accept",
        "topics": counts,
...
def _run_bus_census(repo_root: Path, check: bool, accept: bool) -> int:
    """Report the bus census, gate its structural defects, or write the baseline (a record of
    the counts, not a ratchet: the gate is the defect list, which must be empty)."""
    census = _collect_bus_census(repo_root)
    for name, stats in census["topics"].items():
        print(f"  {stats['publishers']:>3} pub {stats['subscribers']:>3} sub  {name}")
    print(
        f"bus census: {len(census['topics'])} topics, {len(census['errors'])} defect(s)"
    )
    for error in census["errors"]:
        print(f"  {error}", file=sys.stderr)
    if accept:
        if census["errors"]:
            print("bus census: --accept refused while defects stand", file=sys.stderr)
            return 1
        _BUS_CENSUS_BASELINE.write_text(
            json.dumps(census, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
            newline="\n",
        )
        print(f"baseline written to {_BUS_CENSUS_BASELINE}")
        return 0
    if check and census["errors"]:
        return 1
    return 0
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--bus-census",
        action="store_true",
        help="list every Core::Bus topic with its publishers and subscribers (spec 0077)",
    )
    parser.add_argument(
        "--accept",
        action="store_true",
        help="re-baseline the census JSON (with --singleton-census / --tu-census / "
        "--dup-census / --bus-census)",
    )
    parser.add_argument(
        "paths",
        nargs="*",
    ...
    if args.bus_census:
        root = Path(__file__).resolve().parent.parent
        return _run_bus_census(root, check=args.check, accept=args.accept)
