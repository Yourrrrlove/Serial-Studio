...
{
  connect(this,
          &MQTT::Publisher::configurationChanged,
          this,
...
/**
 * @brief Wires the publisher to the notification topic and to ProjectModel so config loads on
...
/**
 * @brief Raw-tap entry for device chunks (spec 0077): the RawRxData lane.
 */
void MQTT::Publisher::onDeviceBytes(int deviceId, const IO::CapturedDataPtr& data)
{
  hotpathTxRawBytes(deviceId, data);
}

/**
 * @brief Frame-tap entry (spec 0077): one call per extracted frame from the pipeline thread.
 */
void MQTT::Publisher::onRawFrame(int deviceId, const IO::CapturedDataPtr& frame)
{
  hotpathTxRawFrame(deviceId, frame);
}

/**
 * @brief Forwards a posted notification to the worker.
 */
void MQTT::Publisher::onNotificationPosted(const Core::Bus::NotificationPosted& posted)
{
  ...
  const QByteArray payload = notificationPayload(posted);
...
{
  Core::Prompt::showMessageBox(ok ? tr("MQTT broker reachable") : tr("MQTT broker unreachable"),
                               detail,
                               ok ? Core::Prompt::Information : Core::Prompt::Critical,
                               tr("MQTT Test Connection"));
}
...
void MQTT::Publisher::onWorkerBrokerError(const QString& message)
{
  if (m_reportConnectionErrors) {
    m_reportConnectionErrors = false;
    Core::Prompt::showMessageBox(
      tr("MQTT broker connection failed"), message, Core::Prompt::Critical, tr("MQTT Publisher"));
  }
}
...
void MQTT::Publisher::reloadTlsIdentity(const bool interactive)
{
  ...
  if (interactive)
    Core::Prompt::showMessageBox(tr("MQTT Client Certificate Error"),
