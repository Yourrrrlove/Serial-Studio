...
#  include "ProjectEditor/Editors/PublisherScriptEditor.h"
#endif
...
/**
 * @brief Binds the facade; the publisher singleton is captured here because this form is its only
 *        reader and writer in the editor.
 */
EditorMqtt::EditorMqtt(ProjectEditor& editor)
  : m_editor(editor)
#ifdef BUILD_COMMERCIAL
  , m_mqttPublisher(API::handlerContext().mqttPublisher)
  , m_mqttScriptEditor(nullptr)
#endif
{}

/**
 * @brief Releases the lazily-opened script editor dialog. It is constructed parentless because the
 *        facade is a QObject and the dialog a QWidget, so no Qt parent chain ever reaches it and
 *        this destructor is its only release path.
 */
EditorMqtt::~EditorMqtt()
{
#ifdef BUILD_COMMERCIAL
 ...
 */
void EditorMqtt::openMqttScriptEditor()
{
#ifdef BUILD_COMMERCIAL
  ...
  if (!m_mqttScriptEditor) {
    ...
    QObject::connect(m_mqttScriptEditor,
 ...
 */
void EditorMqtt::buildMqttPublisherModel()
{
  if (m_editor.m_mqttPublisherModel) {
    m_editor.m_mqttPublisherModel->disconnect(&m_editor);
    m_editor.m_mqttPublisherModel->deleteLater();
    m_editor.m_mqttPublisherModel = nullptr;
  }

  m_editor.m_mqttPublisherModel = new CustomModel(&m_editor);
  ...
  QObject::connect(m_editor.m_mqttPublisherModel,
                   &CustomModel::itemChanged,
                   &m_editor,
                   [this](QStandardItem* item) { onMqttPublisherItemChanged(item); });

  Q_EMIT m_editor.mqttPublisherModelChanged();
}
 ...
 */
void EditorMqtt::buildMqttPublishingSection(const MQTT::Publisher& pub, bool enabled)
{
  ...
  m_editor.m_mqttPublisherModel->appendRow(pubHdr);
  ...
  m_editor.m_mqttPublisherModel->appendRow(enabledItem);
  ...
  modeItem->setData(tr("Selects what gets published: parsed dashboard data or raw RX bytes"),
                    ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(modeItem);
  ...
  freqItem->setData(tr("How many times per second to publish (1-30 Hz). Higher rates increase "
                       ...
                       "frame parsing."),
                    ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(freqItem);
  ...
  m_editor.m_mqttPublisherModel->appendRow(topicItem);
  ...
  if (pub.mode() == static_cast<int>(MQTT::Publisher::Mode::ScriptDriven)) {
    ...
    m_editor.m_mqttPublisherModel->appendRow(scriptTopicItem);
  }
  ...
  notifyItem->setData(tr("Mirror dashboard notifications to a dedicated topic"),
                      ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(notifyItem);
  ...
  if (pub.publishNotifications()) {
    ...
    notifyTopicItem->setData(tr("Topic where dashboard notifications are mirrored"),
                             ParameterDescription);
    m_editor.m_mqttPublisherModel->appendRow(notifyTopicItem);
  }
}
#endif
 ...
 */
void EditorMqtt::buildMqttSparkplugSection(const MQTT::Publisher& pub, bool enabled)
{
  ...
  sparkplugHdr->setData(registry.iconById(QStringLiteral("editor/mqtt-publisher"), 16),
                        ParameterIcon);
  m_editor.m_mqttPublisherModel->appendRow(sparkplugHdr);
  ...
  sparkplugItem->setData(tr("Publish datasets in the Sparkplug B namespace instead of the payload "
                         ...
                         ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(sparkplugItem);
  ...
  groupItem->setData(tr("Logical group &m_editor edge node belongs to"), ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(groupItem);
  ...
  nodeItem->setData(tr("Identifies &m_editor node inside the group"), ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(nodeItem);
  ...
  deviceItem->setData(tr("When set, datasets are published as a device of &m_editor node"),
                      ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(deviceItem);
}
#endif
 ...
 */
void EditorMqtt::buildMqttBrokerSection(const MQTT::Publisher& pub, bool enabled)
{
  ...
  m_editor.m_mqttPublisherModel->appendRow(brokerHdr);
  ...
  m_editor.m_mqttPublisherModel->appendRow(hostItem);
  ...
  portItem->setData(tr("TCP port exposed by the broker (1883 plain, 8883 TLS)"),
                    ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(portItem);
  ...
  customClientIdItem->setData(
    ...
    ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(customClientIdItem);
  ...
  if (pub.customClientId()) {
    ...
    m_editor.m_mqttPublisherModel->appendRow(clientIdItem);
  }
  ...
  m_editor.m_mqttPublisherModel->appendRow(versionItem);
  ...
  m_editor.m_mqttPublisherModel->appendRow(keepAliveItem);
  ...
  m_editor.m_mqttPublisherModel->appendRow(cleanItem);
}
#endif
 ...
 */
void EditorMqtt::buildMqttBrokerCredentials(const MQTT::Publisher& pub, bool enabled)
{
  ...
  userItem->setData(tr("Username for broker authentication (leave empty for anonymous)"),
                    ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(userItem);
  ...
  m_editor.m_mqttPublisherModel->appendRow(passItem);
}
#endif
 ...
 */
void EditorMqtt::buildMqttSslSection(const MQTT::Publisher& pub, bool enabled)
{
  ...
  m_editor.m_mqttPublisherModel->appendRow(sslHdr);
  ...
  m_editor.m_mqttPublisherModel->appendRow(sslItem);
  ...
  m_editor.m_mqttPublisherModel->appendRow(sslProtoItem);
  ...
  peerModeItem->setData(tr("How strictly the broker's certificate chain is validated"),
                        ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(peerModeItem);
  ...
  peerDepthItem->setData(tr("Maximum certificate chain length accepted (0 = unlimited)"),
                         ParameterDescription);
  m_editor.m_mqttPublisherModel->appendRow(peerDepthItem);

  appendMqttClientIdentityRows(m_editor.m_mqttPublisherModel, pub, enabled);
}
#endif
 ...
 */
void EditorMqtt::onMqttPublisherItemChanged(QStandardItem* item)
{
  if (!item)
    return;
