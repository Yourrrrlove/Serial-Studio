...
/**
 * @brief The QObject base a consumer of payload @c T gets: a block consumer is a block sink (spec
 *        0077), so the publisher can hand it blocks through IBlockSink and read its verdict; a
 *        consumer of any other payload (images, console text) is a plain QObject unless it names
 *        IBlockSink itself.
 */
template<typename T>
using FrameConsumerBase = std::conditional_t<std::is_same_v<T, DataBlockPtr>, IBlockSink, QObject>;

/**
 * @brief Main-thread facade that owns a `FrameConsumerWorker` on a dedicated QThread. Provides a
 * lock-free `enqueueData()` hotpath with periodic + threshold-based flushing. @c Base is the
 * QObject it derives from: IBlockSink for block payloads by default, chosen explicitly by a
 * consumer whose payload is something else but which still takes blocks (the audio export).
 */
template<typename T, typename Base = FrameConsumerBase<T>>
class FrameConsumer : public Base {
public:
  /**
