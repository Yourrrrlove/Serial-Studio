...
// Nested under `proto` so these names can't collide with another translation unit's `detail` (ODR).
...
QJsonObject DataModel::ProtoImporter::projectFromProtoFile(const QString& path)
{
  ...
  ProtoParser parser(src);
  if (!parser.parse() || parser.messages().isEmpty())
    return QJsonObject();

  m_messages      = parser.messages();
