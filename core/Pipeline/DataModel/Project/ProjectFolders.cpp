...
/**
 * @brief Finds the next free folder id in @p folders (max + 1, never reusing a hole).
 */
template<typename Folder>
static int nextFolderId(const std::vector<Folder>& folders)
{
  int newId = 1;
  for (const auto& f : std::as_const(folders))
    if (f.folderId >= newId)
...
/**
 * @brief Swaps the folder at @p from with its nearest sibling in @p direction; true when moved.
 */
template<typename Folder>
static bool swapFolderWithSibling(std::vector<Folder>& folders, int folderId, int direction)
{
  const int n = static_cast<int>(folders.size());
  int from    = -1;
  for (int i = 0; i < n; ++i)
    if (folders[static_cast<size_t>(i)].folderId == folderId) {
      from = i;
      break;
    }
  ...
  const int parent = folders[static_cast<size_t>(from)].parentFolderId;
  const int step   = (direction < 0) ? -1 : 1;
  for (int j = from + step; j >= 0 && j < n; j += step) {
    if (folders[static_cast<size_t>(j)].parentFolderId != parent)
      continue;

    std::swap(folders[static_cast<size_t>(from)], folders[static_cast<size_t>(j)]);
    return true;
  }
...
/**
 * @brief Builds an empty folder store bound to @p model.
 */
DataModel::ProjectFolders::ProjectFolders(ProjectModel& model) : m_model(model) {}

/**
 * @brief Returns the workspace folder tree.
 */
const std::vector<DataModel::WorkspaceFolder>& DataModel::ProjectFolders::workspaceFolders()
  const noexcept
{
...
/**
 * @brief Returns the group folder tree.
 */
const std::vector<DataModel::GroupFolder>& DataModel::ProjectFolders::groupFolders() const noexcept
{
  return m_groupFolders;
}

/**
 * @brief Returns the table folder tree.
 */
const std::vector<DataModel::TableFolder>& DataModel::ProjectFolders::tableFolders() const noexcept
{
  return m_tableFolders;
}

/**
 * @brief Mutable workspace folder tree, for the loader and the auto-layout derivation.
 */
std::vector<DataModel::WorkspaceFolder>& DataModel::ProjectFolders::
  mutableWorkspaceFolders() noexcept
{
  return m_workspaceFolders;
}

/**
 * @brief Mutable group folder tree, for the loader.
 */
std::vector<DataModel::GroupFolder>& DataModel::ProjectFolders::mutableGroupFolders() noexcept
{
  return m_groupFolders;
}

/**
 * @brief Mutable table folder tree, for the loader.
 */
std::vector<DataModel::TableFolder>& DataModel::ProjectFolders::mutableTableFolders() noexcept
{
  return m_tableFolders;
}

/**
 * @brief Drops every folder tree (document reset).
 */
void DataModel::ProjectFolders::clearAll()
{
...
/**
 * @brief Drops the workspace folder tree only; the auto layout rederives it from the group tree.
 */
void DataModel::ProjectFolders::clearWorkspaceFolders()
{
  m_workspaceFolders.clear();
}
 ...
 */
QString DataModel::ProjectFolders::workspaceFolderTitle(int folderId) const
{
 ...
 */
int DataModel::ProjectFolders::addWorkspaceFolder(int parentFolderId, const QString& title)
{
  ...
  const int newId = nextFolderId(m_workspaceFolders);

  DataModel::WorkspaceFolder folder;
  folder.folderId       = newId;
  folder.parentFolderId = parentFolderId;
  folder.title = title.simplified().isEmpty() ? ProjectModel::tr("Folder") : title.simplified();
 ...
 */
void DataModel::ProjectFolders::renameWorkspaceFolder(int folderId, const QString& title)
{
...
void DataModel::ProjectFolders::moveWorkspaceInFolder(int workspaceId, int direction)
{
  ...
  for (int j = from + step; j >= 0 && j < n; j += step) {
    ...
    std::swap(workspaces[static_cast<size_t>(from)], workspaces[static_cast<size_t>(j)]);
    m_model.setModified(true);
    Q_EMIT m_model.editorWorkspacesChanged();
    Q_EMIT m_model.activeWorkspacesChanged();
    return;
 ...
 */
void DataModel::ProjectFolders::promptRenameWorkspaceFolder(int folderId)
{
 ...
 */
void DataModel::ProjectFolders::confirmDeleteWorkspaceFolder(int folderId)
{
...
//--------------------------------------------------------------------------------------------------
// Group folder CRUD (editor-only organization; never reorders the group list)
//--------------------------------------------------------------------------------------------------

/**
 * @brief Returns the title of a group folder, or empty if not found.
 */
QString DataModel::ProjectFolders::groupFolderTitle(int folderId) const
{
 ...
 */
int DataModel::ProjectFolders::addGroupFolder(int parentFolderId, const QString& title)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Add Folder")};
  if (parentFolderId != -1 && !folderExists(m_groupFolders, parentFolderId))
    parentFolderId = -1;

  const int newId = nextFolderId(m_groupFolders);
 ...
 */
void DataModel::ProjectFolders::renameGroupFolder(int folderId, const QString& title)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Rename Folder")};
  if (title.simplified().isEmpty())
    return;
 ...
 */
void DataModel::ProjectFolders::deleteGroupFolder(int folderId)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Delete Folder")};
...
/**
 * @brief Files a group into folder @p parentFolderId (-1 = top level); never reorders the groups.
 */
void DataModel::ProjectFolders::moveGroupToFolder(int groupId, int parentFolderId)
{
  ...
  auto& group = m_model.m_groups[static_cast<size_t>(groupId)];
 ...
 */
void DataModel::ProjectFolders::moveGroupFolderToFolder(int folderId, int parentFolderId)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Move Folder")};
  if (parentFolderId != -1 && !folderExists(m_groupFolders, parentFolderId))
    return;
 ...
 */
void DataModel::ProjectFolders::moveGroupFolderInParent(int folderId, int direction)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Move Folder")};
  if (!swapFolderWithSibling(m_groupFolders, folderId, direction))
    return;
 ...
 */
void DataModel::ProjectFolders::promptAddGroupFolder(int parentFolderId)
{
  bool ok            = false;
  const QString name = QInputDialog::getText(nullptr,
                                             ProjectModel::tr("New Folder"),
 ...
 */
void DataModel::ProjectFolders::promptRenameGroupFolder(int folderId)
{
 ...
 */
void DataModel::ProjectFolders::confirmDeleteGroupFolder(int folderId)
{
 ...
 */
QString DataModel::ProjectFolders::tableFolderTitle(int folderId) const
{
 ...
 */
int DataModel::ProjectFolders::addTableFolder(int parentFolderId, const QString& title)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Add Folder")};
  if (parentFolderId != -1 && !folderExists(m_tableFolders, parentFolderId))
    parentFolderId = -1;

  const int newId = nextFolderId(m_tableFolders);
  ...
  if (folder.title.isEmpty())
    folder.title = ProjectModel::tr("Folder");
 ...
 */
void DataModel::ProjectFolders::renameTableFolder(int folderId, const QString& title)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Rename Folder")};
 ...
 */
void DataModel::ProjectFolders::deleteTableFolder(int folderId)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Delete Folder")};
  ...
  for (auto& t : m_model.m_tables.mutableList())
 ...
 */
void DataModel::ProjectFolders::moveTableToFolder(const QString& tablePath, int parentFolderId)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Move Table")};
  if (parentFolderId != -1 && !folderExists(m_tableFolders, parentFolderId))
    return;

  const int idx = m_model.m_tables.findTableIndexByPath(tablePath);
  if (idx < 0)
    return;

  auto& tables = m_model.m_tables.mutableList();
  auto& table  = tables[static_cast<size_t>(idx)];
  if (table.parentFolderId == parentFolderId)
    return;

  for (size_t i = 0; i < tables.size(); ++i)
    if (static_cast<int>(i) != idx && tables[i].parentFolderId == parentFolderId
 ...
 */
void DataModel::ProjectFolders::moveTableFolderToFolder(int folderId, int parentFolderId)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Move Folder")};
  if (parentFolderId != -1 && !folderExists(m_tableFolders, parentFolderId))
    return;
 ...
 */
void DataModel::ProjectFolders::moveTableFolderInParent(int folderId, int direction)
{
  const ProjectUndoScope undo_scope{m_model, ProjectModel::tr("Move Folder")};
  if (!swapFolderWithSibling(m_tableFolders, folderId, direction))
    return;
 ...
 */
void DataModel::ProjectFolders::promptAddTableFolder(int parentFolderId)
{
  bool ok            = false;
  const QString name = QInputDialog::getText(nullptr,
                                             ProjectModel::tr("New Folder"),
 ...
 */
void DataModel::ProjectFolders::promptRenameTableFolder(int folderId)
{
 ...
 */
void DataModel::ProjectFolders::confirmDeleteTableFolder(int folderId)
{
 ...
 * object (the list is merged in place, never rebuilt), so this only guards hand-edited files.
 */
void DataModel::ProjectFolders::sanitizeWorkspaceFolders()
{
  sanitizeFolderTree(m_workspaceFolders, m_model.m_workspaces.mutableList());
}

/**
 * @brief Repairs the group folder tree after load (cyclic/dangling parents -> top level).
 */
void DataModel::ProjectFolders::sanitizeGroupFolders()
{
  sanitizeFolderTree(m_groupFolders, m_model.m_groups);
}

/**
 * @brief Repairs the table folder tree after load (cyclic/dangling parents -> top level).
 */
void DataModel::ProjectFolders::sanitizeTableFolders()
{
  sanitizeFolderTree(m_tableFolders, m_model.m_tables.mutableList());
}
 ...
 */
QSet<int> DataModel::ProjectFolders::duplicateGroupFolderSubtree(int rootFolderId)
{
  ...
  for (const int gid : groupIds) {
    const int oldParent = m_model.m_groups[static_cast<size_t>(gid)].parentFolderId;
    m_model.duplicateGroup(gid);
    m_model.m_groups.back().parentFolderId = idMap.value(oldParent, -1);
  }
 ...
 */
QSet<int> DataModel::ProjectFolders::duplicateTableFolderSubtree(int rootFolderId)
{
  ...
  for (const auto& t : m_model.m_tables.list())
  ...
  for (size_t i = 0; i < copies.size(); ++i)
    m_model.m_tables.appendTableCopyToFolder(copies[i], targetParents[i]);
